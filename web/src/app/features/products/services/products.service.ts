...
export class ProductsService {
  private readonly apiUrl = `${window.__env.NG_APP_API_URL}/products`;
  ...
  getProductById(id: string): Observable<Product> {
    return this.http.get<Product>(`${window.__env.NG_APP_API_URL}/${id}`);
  }
  ...
  suggestSku(): Observable<{ sku: string }> {
    return this.http.get<{ sku: string }>(`${window.__env.NG_APP_API_URL}/sku-suggest`);
  }
  ...
  updateProduct(id: string, request: UpdateProductRequest): Observable<Product> {
    return this.http.put<Product>(`${window.__env.NG_APP_API_URL}/${id}`, request);
  }
