...
export class BatchEditComponent implements OnInit {
  ...
  loadBatchData() {
    this.inventoryService.getBatch(this.batchId).subscribe({
      next: (batch) => {
  ...
  saveBatch() {
    if (this.batchForm.valid) {
      ...
      const batchData = {
        ...
        location: 'Dry Storage A1', // TODO: Make this editable
        status: this.batchForm.value.status
      };

      this.inventoryService.updateBatch(this.batchId, batchData).subscribe({
        next: () => {
          this.router.navigate(['/inventory/batches']);
        },
        error: (err) => {
