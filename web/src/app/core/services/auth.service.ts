...
export class AuthService {
  ...
  getMyCompanyProfile(): Observable<CompanyProfile> {
    return this.http.get<CompanyProfile>(`${this.apiBaseUrl}/organization/my-profile`);
  }

  updateMyCompanyProfile(request: UpdateCompanyProfileRequest): Observable<{ message: string }> {
    return this.http.put<{ message: string }>(`${this.apiBaseUrl}/organization/my-profile`, request).pipe(
      tap(() => {
        const current = this.currentUserValue;
        if (!current) {
          return;
