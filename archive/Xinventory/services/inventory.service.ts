...
export class InventoryService {
  ...
  getCompositeInventory(): Observable<InventoryItem[]> {
    return this.http.get<InventoryItemApi[]>(`${window.__env.NG_APP_API_URL}/composite-list`).pipe(
      map(items => items.map(item => this.mapApiToFrontend(item)))
    );
  }
  ...
  getInventoryItemById(id: string): Observable<InventoryItem> {
    return this.http.get<InventoryItemApi>(`${window.__env.NG_APP_API_URL}/${id}`).pipe(
  ...
  getInventoryBySupplier(supplierId: string): Observable<InventoryItem[]> {
    return this.http.get<InventoryItemApi[]>(`${window.__env.NG_APP_API_URL}/supplier/${supplierId}`).pipe(
      map(items => items.map(item => this.mapApiToFrontend(item)))
    );
  }
  ...
  getItemCategories(): Observable<InventoryCategory[]> {
    return this.http.get<InventoryCategoryApi[]>(`${window.__env.NG_APP_API_URL}/item-categories`).pipe(
  ...
  getProductCategories(): Observable<ProductCategory[]> {
    return this.http.get<ProductCategoryApi[]>(`${window.__env.NG_APP_API_URL}/product-categories`).pipe(
  ...
  getRecipeComponents(productCategories?: ProductCategory[]): Observable<InventoryItem[]> {
    if (productCategories) {
      return this.http.get<InventoryRecipeComponentApi[]>(`${window.__env.NG_APP_API_URL}/recipe-components`).pipe(
      ...
      return this.getProductCategories().pipe(
        switchMap(productCategoryList =>
          this.http.get<InventoryRecipeComponentApi[]>(`${window.__env.NG_APP_API_URL}/recipe-components`).pipe(
  ...
  getLowStockItems(): Observable<InventoryItem[]> {
    return this.http.get<InventoryItemApi[]>(`${window.__env.NG_APP_API_URL}/low-stock`).pipe(
      map(items => items.map(item => this.mapApiToFrontend(item)))
    );
  }
  ...
  getExpiringItems(daysUntilExpiration: number = 7): Observable<InventoryItem[]> {
    return this.http.get<InventoryItemApi[]>(`${window.__env.NG_APP_API_URL}/expiring-soon`, {
  ...
  reserveInventory(inventoryItemId: string, quantity: number, referenceId: string): Observable<AvailabilityResponse> {
    ...
    return this.http.post<AvailabilityResponse>(`${window.__env.NG_APP_API_URL}/${inventoryItemId}/reserve`, request);
  }
